...
void cConstrainToPoint::constrainToPoint(cVector3d a_point) {
    ...
    m_targetSphere->setLocalPos(m_point);
...
bool cEffectConstrainToPoint::computeForce(const cVector3d& a_toolPos,
    ...
    if(parent->m_active) {
        double stiffness = 0.7 * parent->m_material->getStiffness(); //added a scalar 0.8/0.7 to reduce the spring force when arriving at target.
