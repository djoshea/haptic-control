...
cPlanarObstacle::cPlanarObstacle(cWorld* a_world, double a_height, const std::vector<cVector3d> a_points) :
m_height(a_height),
m_sticky(false),
m_closed(true)
{
    ...
    // for visualizing trap direction std::vector
    ...
    // for visualizing trap spring std::vector
...
void cPlanarObstacle::setHapticProperties() {
    m_surface->setEnabled(true);
    //m_magnet->setEnabled(false);
    //m_vibration->setEnabled(false);
    m_viscosity->setEnabled(false);
    m_collisionTrap->setEnabled(true);
...
void cPlanarObstacle::setPoints(std::vector<cVector3d> a_points) {
    m_points = std::vector<cVector3d>(a_points);
...
int cPlanarObstacle::findPolygonPointMatchingVector2d(const Vector2d &pt) {
    unsigned nPoints = m_points.size();
    for(unsigned i = 0; i < nPoints; i++) {
        if(fabs(pt.GetX() - m_points[i].x()) < 0.001 &&
           fabs(pt.GetY() - m_points[i].y()) < 0.001)
            return i;
    }
                                  ...
                                  cVector3d& a_reactionForce)
{
    ...
    if (m_trapTriggered) {
        ...
        decomposeVector3d(vRestoring, cSub(parent->m_trapLateralB, parent->m_trapLateralA), vRestoringLateral, perp);
