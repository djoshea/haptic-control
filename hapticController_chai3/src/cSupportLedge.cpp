...
void cSupportLedge::render(cRenderOptions &a_options)
{
    m_planeMesh->renderSceneGraph(a_options);
}
                                  ...
                                  cVector3d& a_reactionForce)
{
    ...
    if(fabs(a_toolPos.x()) <= parent->m_width / 2.0 && a_toolPos.y() < 0 && fabs(a_toolPos.z()) <= parent->m_height/2.0) {
        // pushing on plane
        goalPos.y(0);
